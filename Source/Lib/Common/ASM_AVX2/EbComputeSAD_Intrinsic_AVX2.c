...
#define EIGHT_SAD_REDUCE_LOADS
#define EIGHT_SAD_CALC_16BIT_SAD
...
#ifdef EIGHT_SAD_CALC_16BIT_SAD
    DECLARE_ALIGNED(32, uint16_t, sad_16x8[32][8]);
    DECLARE_ALIGNED(32, uint16_t, sad_8x16[32][8]);
    DECLARE_ALIGNED(32, uint16_t, sad_32x16[8][8]);
    DECLARE_ALIGNED(32, uint16_t, sad_16x32[8][8]);
    DECLARE_ALIGNED(32, uint16_t, sad_16bit[8]);
    __m256i temp_sad;
    __m128i s3;
#ifdef EIGHT_SAD_REDUCE_LOADS
    __m128i pack[4];
#endif
#else
...
#ifdef EIGHT_SAD_REDUCE_LOADS
    DECLARE_ALIGNED(32, uint32_t, sad_64x32[2][8]);
    DECLARE_ALIGNED(32, uint32_t, sad_32x64[2][8]);
...
#define ADD_VECS_16bit_SAD(dst, a, b)                 \
    *(__m128i *) dst = _mm_add_epi16(                 \
        *(__m128i const *) a,         \
        *(__m128i const *) b);

#define ADD_VECS_16BIT_SAD_PACK(dst, a, b)            \
     temp_sad = _mm256_adds_epu16(                    \
        _mm256_loadu_si256((__m256i const *) a),      \
        _mm256_loadu_si256((__m256i const *) b));     \
    temp_sad = _mm256_packs_epi32(                    \
        temp_sad,                                     \
        _mm256_setr_m128i(                            \
            _mm256_extractf128_si256(temp_sad,1),     \
            _mm_setzero_si128()));                    \
    *(__m128i *) dst = _mm256_extractf128_si256(temp_sad, 0);

/* Load to two registers __m128i */
#define LOAD_16BIT_PACK_2(dst, a, b)                  \
    temp_sad = _mm256_packs_epi32(                    \
        _mm256_loadu_si256((__m256i const *) a),      \
        _mm256_loadu_si256((__m256i const *) b));     \
    *(__m256i *) &dst = _mm256_permute4x64_epi64(temp_sad, 0xD8);

#define SEARCH_BEST_16BIT_SAD(serch, best, best_mv)   \
       s3 = _mm_minpos_epu16(*(__m128i*)serch);       \
       search_index = _mm_extract_epi16(s3, 1);       \
    if (serch[search_index] < best) {                 \
        best = serch[search_index];                   \
        best_mv = computed_idx[search_index];         \
    }

#define SEARCH_CALC_BEST_16BIT_SAD(a, b, best, best_mv) \
    ADD_VECS_16bit_SAD(sad_16bit, a, b);              \
    SEARCH_BEST_16BIT_SAD(sad_16bit, best, best_mv)

#else /* EIGHT_SAD_CALC_16BIT_SAD */

#define ADD_VECS_16BIT_SAD_PACK         ADD_VECS
#define SEARCH_BEST_16BIT_SAD           SEARCH_BEST
#define SEARCH_CALC_BEST_16BIT_SAD      SEARCH_CALC_BEST

#endif /* EIGHT_SAD_CALC_16BIT_SAD */

#ifdef EIGHT_SAD_REDUCE_LOADS
#ifdef EIGHT_SAD_CALC_16BIT_SAD

    LOAD_16BIT_PACK_2(pack[0], p_sad16x16[0], p_sad16x16[1]);
    LOAD_16BIT_PACK_2(pack[2], p_sad16x16[2], p_sad16x16[3]);
    *(__m128i *) sad_32x16[0] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_16x32[0] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_16x32[1] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_32x16[1] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad16x16[4], p_sad16x16[5]);
    LOAD_16BIT_PACK_2(pack[2], p_sad16x16[6], p_sad16x16[7]);
    *(__m128i *) sad_32x16[2] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_16x32[2] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_16x32[3] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_32x16[3] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad16x16[8], p_sad16x16[9]);
    LOAD_16BIT_PACK_2(pack[2], p_sad16x16[10], p_sad16x16[11]);
    *(__m128i *) sad_32x16[4] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_16x32[4] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_16x32[5] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_32x16[5] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad16x16[12], p_sad16x16[13]);
    LOAD_16BIT_PACK_2(pack[2], p_sad16x16[14], p_sad16x16[15]);
    *(__m128i *) sad_32x16[6] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_16x32[6] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_16x32[7] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_32x16[7] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[0], p_sad8x8[1]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[2], p_sad8x8[3]);
    *(__m128i *) sad_16x8[0] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[0] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[1] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[1] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[4], p_sad8x8[5]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[6], p_sad8x8[7]);
    *(__m128i *) sad_16x8[2] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[2] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[3] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[3] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[8], p_sad8x8[9]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[10], p_sad8x8[11]);
    *(__m128i *) sad_16x8[4] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[4] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[5] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[5] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[12], p_sad8x8[13]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[14], p_sad8x8[15]);
    *(__m128i *) sad_16x8[6] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[6] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[7] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[7] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[16], p_sad8x8[17]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[18], p_sad8x8[19]);
    *(__m128i *) sad_16x8[8] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[8] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[9] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[9] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[20], p_sad8x8[21]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[22], p_sad8x8[23]);
    *(__m128i *) sad_16x8[10] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[10] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[11] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[11] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[24], p_sad8x8[25]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[26], p_sad8x8[27]);
    *(__m128i *) sad_16x8[12] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[12] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[13] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[13] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[28], p_sad8x8[29]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[30], p_sad8x8[31]);
    *(__m128i *) sad_16x8[14] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[14] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[15] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[15] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[32], p_sad8x8[33]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[34], p_sad8x8[35]);
    *(__m128i *) sad_16x8[16] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[16] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[17] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[17] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[36], p_sad8x8[37]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[38], p_sad8x8[39]);
    *(__m128i *) sad_16x8[18] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[18] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[19] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[19] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[40], p_sad8x8[41]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[42], p_sad8x8[43]);
    *(__m128i *) sad_16x8[20] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[20] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[21] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[21] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[44], p_sad8x8[45]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[46], p_sad8x8[47]);
    *(__m128i *) sad_16x8[22] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[22] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[23] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[23] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[48], p_sad8x8[49]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[50], p_sad8x8[51]);
    *(__m128i *) sad_16x8[24] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[24] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[25] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[25] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[52], p_sad8x8[53]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[54], p_sad8x8[55]);
    *(__m128i *) sad_16x8[26] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[26] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[27] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[27] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[56], p_sad8x8[57]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[58], p_sad8x8[59]);
    *(__m128i *) sad_16x8[28] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[28] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[29] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[29] = _mm_add_epi16(pack[2], pack[3]);
    LOAD_16BIT_PACK_2(pack[0], p_sad8x8[60], p_sad8x8[61]);
    LOAD_16BIT_PACK_2(pack[2], p_sad8x8[62], p_sad8x8[63]);
    *(__m128i *) sad_16x8[30] = _mm_add_epi16(pack[0], pack[1]);
    *(__m128i *) sad_8x16[30] = _mm_add_epi16(pack[0], pack[2]);
    *(__m128i *) sad_8x16[31] = _mm_add_epi16(pack[1], pack[3]);
    *(__m128i *) sad_16x8[31] = _mm_add_epi16(pack[2], pack[3]);

#else /* EIGHT_SAD_CALC_16BIT_SAD */
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad16x16[0]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad16x16[1]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad16x16[2]);
    *(__m256i *)sad_32x16[0] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_16x32[0] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad16x16[3]);
    *(__m256i *)sad_32x16[1] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_16x32[1] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad16x16[4]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad16x16[5]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad16x16[6]);
    *(__m256i *)sad_32x16[2] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_16x32[2] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad16x16[7]);
    *(__m256i *)sad_32x16[3] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_16x32[3] = _mm256_add_epi32(tmp3, tmp2);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad16x16[8]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad16x16[9]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad16x16[10]);
    *(__m256i *)sad_32x16[4] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_16x32[4] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad16x16[11]);
    *(__m256i *)sad_32x16[5] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_16x32[5] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad16x16[12]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad16x16[13]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad16x16[14]);
    *(__m256i *)sad_32x16[6] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_16x32[6] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad16x16[15]);
    *(__m256i *)sad_32x16[7] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_16x32[7] = _mm256_add_epi32(tmp3, tmp2);

    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[0]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[1]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[2]);
    *(__m256i *)sad_16x8[0] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[0] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[3]);
    *(__m256i *)sad_16x8[1] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[1] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[4]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[5]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[6]);
    *(__m256i *)sad_16x8[2] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[2] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[7]);
    *(__m256i *)sad_16x8[3] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[3] = _mm256_add_epi32(tmp3, tmp2);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[8]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[9]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[10]);
    *(__m256i *)sad_16x8[4] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[4] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[11]);
    *(__m256i *)sad_16x8[5] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[5] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[12]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[13]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[14]);
    *(__m256i *)sad_16x8[6] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[6] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[15]);
    *(__m256i *)sad_16x8[7] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[7] = _mm256_add_epi32(tmp3, tmp2);

    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[16]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[17]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[18]);
    *(__m256i *)sad_16x8[8] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[8] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[19]);
    *(__m256i *)sad_16x8[9] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[9] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[20]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[21]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[22]);
    *(__m256i *)sad_16x8[10] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[10] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[23]);
    *(__m256i *)sad_16x8[11] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[11] = _mm256_add_epi32(tmp3, tmp2);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[24]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[25]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[26]);
    *(__m256i *)sad_16x8[12] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[12] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[27]);
    *(__m256i *)sad_16x8[13] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[13] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[28]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[29]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[30]);
    *(__m256i *)sad_16x8[14] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[14] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[31]);
    *(__m256i *)sad_16x8[15] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[15] = _mm256_add_epi32(tmp3, tmp2);

    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[32]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[33]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[34]);
    *(__m256i *)sad_16x8[16] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[16] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[35]);
    *(__m256i *)sad_16x8[17] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[17] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[36]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[37]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[38]);
    *(__m256i *)sad_16x8[18] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[18] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[39]);
    *(__m256i *)sad_16x8[19] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[19] = _mm256_add_epi32(tmp3, tmp2);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[40]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[41]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[42]);
    *(__m256i *)sad_16x8[20] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[20] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[43]);
    *(__m256i *)sad_16x8[21] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[21] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[44]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[45]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[46]);
    *(__m256i *)sad_16x8[22] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[22] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[47]);
    *(__m256i *)sad_16x8[23] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[23] = _mm256_add_epi32(tmp3, tmp2);

    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[48]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[49]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[50]);
    *(__m256i *)sad_16x8[24] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[24] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[51]);
    *(__m256i *)sad_16x8[25] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[25] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[52]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[53]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[54]);
    *(__m256i *)sad_16x8[26] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[26] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[55]);
    *(__m256i *)sad_16x8[27] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[27] = _mm256_add_epi32(tmp3, tmp2);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[56]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[57]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[58]);
    *(__m256i *)sad_16x8[28] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[28] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[59]);
    *(__m256i *)sad_16x8[29] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_8x16[29] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[60]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad8x8[61]);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad8x8[62]);
    *(__m256i *)sad_16x8[30] = _mm256_add_epi32(tmp2, tmp3);
    *(__m256i *)sad_8x16[30] = _mm256_add_epi32(tmp2, tmp1);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad8x8[63]);
    *(__m256i *)sad_16x8[31] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_8x16[31] = _mm256_add_epi32(tmp3, tmp2);

#endif /* EIGHT_SAD_CALC_16BIT_SAD */

#else /* EIGHT_SAD_REDUCE_LOADS */
    // 32x16
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[0], p_sad16x16[0], p_sad16x16[1]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[1], p_sad16x16[2], p_sad16x16[3]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[2], p_sad16x16[4], p_sad16x16[5]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[3], p_sad16x16[6], p_sad16x16[7]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[4], p_sad16x16[8], p_sad16x16[9]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[5], p_sad16x16[10], p_sad16x16[11]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[6], p_sad16x16[12], p_sad16x16[13]);
    ADD_VECS_16BIT_SAD_PACK(sad_32x16[7], p_sad16x16[14], p_sad16x16[15]);

    // 16x8
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[0], p_sad8x8[0], p_sad8x8[1]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[1], p_sad8x8[2], p_sad8x8[3]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[2], p_sad8x8[4], p_sad8x8[5]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[3], p_sad8x8[6], p_sad8x8[7]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[4], p_sad8x8[8], p_sad8x8[9]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[5], p_sad8x8[10], p_sad8x8[11]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[6], p_sad8x8[12], p_sad8x8[13]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[7], p_sad8x8[14], p_sad8x8[15]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[8], p_sad8x8[16], p_sad8x8[17]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[9], p_sad8x8[18], p_sad8x8[19]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[10], p_sad8x8[20], p_sad8x8[21]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[11], p_sad8x8[22], p_sad8x8[23]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[12], p_sad8x8[24], p_sad8x8[25]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[13], p_sad8x8[26], p_sad8x8[27]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[14], p_sad8x8[28], p_sad8x8[29]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[15], p_sad8x8[30], p_sad8x8[31]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[16], p_sad8x8[32], p_sad8x8[33]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[17], p_sad8x8[34], p_sad8x8[35]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[18], p_sad8x8[36], p_sad8x8[37]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[19], p_sad8x8[38], p_sad8x8[39]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[20], p_sad8x8[40], p_sad8x8[41]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[21], p_sad8x8[42], p_sad8x8[43]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[22], p_sad8x8[44], p_sad8x8[45]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[23], p_sad8x8[46], p_sad8x8[47]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[24], p_sad8x8[48], p_sad8x8[49]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[25], p_sad8x8[50], p_sad8x8[51]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[26], p_sad8x8[52], p_sad8x8[53]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[27], p_sad8x8[54], p_sad8x8[55]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[28], p_sad8x8[56], p_sad8x8[57]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[29], p_sad8x8[58], p_sad8x8[59]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[30], p_sad8x8[60], p_sad8x8[61]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x8[31], p_sad8x8[62], p_sad8x8[63]);
    // 16x32
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[0], p_sad16x16[0], p_sad16x16[2]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[1], p_sad16x16[1], p_sad16x16[3]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[2], p_sad16x16[4], p_sad16x16[6]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[3], p_sad16x16[5], p_sad16x16[7]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[4], p_sad16x16[8], p_sad16x16[10]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[5], p_sad16x16[9], p_sad16x16[11]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[6], p_sad16x16[12], p_sad16x16[14]);
    ADD_VECS_16BIT_SAD_PACK(sad_16x32[7], p_sad16x16[13], p_sad16x16[15]);
    // 8x16
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[0], p_sad8x8[0], p_sad8x8[2]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[1], p_sad8x8[1], p_sad8x8[3]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[2], p_sad8x8[4], p_sad8x8[6]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[3], p_sad8x8[5], p_sad8x8[7]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[4], p_sad8x8[8], p_sad8x8[10]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[5], p_sad8x8[9], p_sad8x8[11]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[6], p_sad8x8[12], p_sad8x8[14]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[7], p_sad8x8[13], p_sad8x8[15]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[8], p_sad8x8[16], p_sad8x8[18]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[9], p_sad8x8[17], p_sad8x8[19]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[10], p_sad8x8[20], p_sad8x8[22]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[11], p_sad8x8[21], p_sad8x8[23]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[12], p_sad8x8[24], p_sad8x8[26]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[13], p_sad8x8[25], p_sad8x8[27]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[14], p_sad8x8[28], p_sad8x8[30]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[15], p_sad8x8[29], p_sad8x8[31]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[16], p_sad8x8[32], p_sad8x8[34]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[17], p_sad8x8[33], p_sad8x8[35]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[18], p_sad8x8[36], p_sad8x8[38]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[19], p_sad8x8[37], p_sad8x8[39]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[20], p_sad8x8[40], p_sad8x8[42]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[21], p_sad8x8[41], p_sad8x8[43]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[22], p_sad8x8[44], p_sad8x8[46]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[23], p_sad8x8[45], p_sad8x8[47]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[24], p_sad8x8[48], p_sad8x8[50]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[25], p_sad8x8[49], p_sad8x8[51]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[26], p_sad8x8[52], p_sad8x8[54]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[27], p_sad8x8[53], p_sad8x8[55]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[28], p_sad8x8[56], p_sad8x8[58]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[29], p_sad8x8[57], p_sad8x8[59]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[30], p_sad8x8[60], p_sad8x8[62]);
    ADD_VECS_16BIT_SAD_PACK(sad_8x16[31], p_sad8x8[61], p_sad8x8[63]);
#endif  /* EIGHT_SAD_REDUCE_LOADS */
    // 32x16
    SEARCH_BEST_16BIT_SAD(sad_32x16[0], p_best_sad32x16[0], p_best_mv32x16[0]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[1], p_best_sad32x16[1], p_best_mv32x16[1]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[2], p_best_sad32x16[2], p_best_mv32x16[2]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[3], p_best_sad32x16[3], p_best_mv32x16[3]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[4], p_best_sad32x16[4], p_best_mv32x16[4]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[5], p_best_sad32x16[5], p_best_mv32x16[5]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[6], p_best_sad32x16[6], p_best_mv32x16[6]);
    SEARCH_BEST_16BIT_SAD(sad_32x16[7], p_best_sad32x16[7], p_best_mv32x16[7]);
    // 16x8
    SEARCH_BEST_16BIT_SAD(sad_16x8[0], p_best_sad16x8[0], p_best_mv16x8[0]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[1], p_best_sad16x8[1], p_best_mv16x8[1]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[2], p_best_sad16x8[2], p_best_mv16x8[2]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[3], p_best_sad16x8[3], p_best_mv16x8[3]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[4], p_best_sad16x8[4], p_best_mv16x8[4]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[5], p_best_sad16x8[5], p_best_mv16x8[5]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[6], p_best_sad16x8[6], p_best_mv16x8[6]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[7], p_best_sad16x8[7], p_best_mv16x8[7]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[8], p_best_sad16x8[8], p_best_mv16x8[8]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[9], p_best_sad16x8[9], p_best_mv16x8[9]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[10], p_best_sad16x8[10], p_best_mv16x8[10]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[11], p_best_sad16x8[11], p_best_mv16x8[11]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[12], p_best_sad16x8[12], p_best_mv16x8[12]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[13], p_best_sad16x8[13], p_best_mv16x8[13]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[14], p_best_sad16x8[14], p_best_mv16x8[14]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[15], p_best_sad16x8[15], p_best_mv16x8[15]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[16], p_best_sad16x8[16], p_best_mv16x8[16]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[17], p_best_sad16x8[17], p_best_mv16x8[17]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[18], p_best_sad16x8[18], p_best_mv16x8[18]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[19], p_best_sad16x8[19], p_best_mv16x8[19]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[20], p_best_sad16x8[20], p_best_mv16x8[20]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[21], p_best_sad16x8[21], p_best_mv16x8[21]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[22], p_best_sad16x8[22], p_best_mv16x8[22]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[23], p_best_sad16x8[23], p_best_mv16x8[23]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[24], p_best_sad16x8[24], p_best_mv16x8[24]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[25], p_best_sad16x8[25], p_best_mv16x8[25]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[26], p_best_sad16x8[26], p_best_mv16x8[26]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[27], p_best_sad16x8[27], p_best_mv16x8[27]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[28], p_best_sad16x8[28], p_best_mv16x8[28]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[29], p_best_sad16x8[29], p_best_mv16x8[29]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[30], p_best_sad16x8[30], p_best_mv16x8[30]);
    SEARCH_BEST_16BIT_SAD(sad_16x8[31], p_best_sad16x8[31], p_best_mv16x8[31]);
    // 16x32
    SEARCH_BEST_16BIT_SAD(sad_16x32[0], p_best_sad16x32[0], p_best_mv16x32[0]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[1], p_best_sad16x32[1], p_best_mv16x32[1]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[2], p_best_sad16x32[2], p_best_mv16x32[2]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[3], p_best_sad16x32[3], p_best_mv16x32[3]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[4], p_best_sad16x32[4], p_best_mv16x32[4]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[5], p_best_sad16x32[5], p_best_mv16x32[5]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[6], p_best_sad16x32[6], p_best_mv16x32[6]);
    SEARCH_BEST_16BIT_SAD(sad_16x32[7], p_best_sad16x32[7], p_best_mv16x32[7]);
    // 8x16
    SEARCH_BEST_16BIT_SAD(sad_8x16[0], p_best_sad8x16[0], p_best_mv8x16[0]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[1], p_best_sad8x16[1], p_best_mv8x16[1]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[2], p_best_sad8x16[2], p_best_mv8x16[2]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[3], p_best_sad8x16[3], p_best_mv8x16[3]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[4], p_best_sad8x16[4], p_best_mv8x16[4]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[5], p_best_sad8x16[5], p_best_mv8x16[5]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[6], p_best_sad8x16[6], p_best_mv8x16[6]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[7], p_best_sad8x16[7], p_best_mv8x16[7]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[8], p_best_sad8x16[8], p_best_mv8x16[8]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[9], p_best_sad8x16[9], p_best_mv8x16[9]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[10], p_best_sad8x16[10], p_best_mv8x16[10]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[11], p_best_sad8x16[11], p_best_mv8x16[11]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[12], p_best_sad8x16[12], p_best_mv8x16[12]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[13], p_best_sad8x16[13], p_best_mv8x16[13]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[14], p_best_sad8x16[14], p_best_mv8x16[14]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[15], p_best_sad8x16[15], p_best_mv8x16[15]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[16], p_best_sad8x16[16], p_best_mv8x16[16]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[17], p_best_sad8x16[17], p_best_mv8x16[17]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[18], p_best_sad8x16[18], p_best_mv8x16[18]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[19], p_best_sad8x16[19], p_best_mv8x16[19]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[20], p_best_sad8x16[20], p_best_mv8x16[20]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[21], p_best_sad8x16[21], p_best_mv8x16[21]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[22], p_best_sad8x16[22], p_best_mv8x16[22]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[23], p_best_sad8x16[23], p_best_mv8x16[23]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[24], p_best_sad8x16[24], p_best_mv8x16[24]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[25], p_best_sad8x16[25], p_best_mv8x16[25]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[26], p_best_sad8x16[26], p_best_mv8x16[26]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[27], p_best_sad8x16[27], p_best_mv8x16[27]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[28], p_best_sad8x16[28], p_best_mv8x16[28]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[29], p_best_sad8x16[29], p_best_mv8x16[29]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[30], p_best_sad8x16[30], p_best_mv8x16[30]);
    SEARCH_BEST_16BIT_SAD(sad_8x16[31], p_best_sad8x16[31], p_best_mv8x16[31]);

#ifdef EIGHT_SAD_REDUCE_LOADS
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad32x32[0]);
    tmp2 = _mm256_loadu_si256((__m256i const *)p_sad32x32[1]);
    tmp3 = _mm256_loadu_si256((__m256i const *)p_sad32x32[2]);
    *(__m256i *)sad_64x32[0] = _mm256_add_epi32(tmp1, tmp2);
    *(__m256i *)sad_32x64[0] = _mm256_add_epi32(tmp1, tmp3);
    tmp1 = _mm256_loadu_si256((__m256i const *)p_sad32x32[3]);
    *(__m256i *)sad_64x32[1] = _mm256_add_epi32(tmp3, tmp1);
    *(__m256i *)sad_32x64[1] = _mm256_add_epi32(tmp2, tmp1);

    SEARCH_BEST(sad_64x32[0], p_best_sad64x32[0], p_best_mv64x32[0]);
    SEARCH_BEST(sad_64x32[1], p_best_sad64x32[1], p_best_mv64x32[1]);
    SEARCH_BEST(sad_32x64[0], p_best_sad32x64[0], p_best_mv32x64[0]);
    SEARCH_BEST(sad_32x64[1], p_best_sad32x64[1], p_best_mv32x64[1]);
#else
    ...
    SEARCH_CALC_BEST(p_sad32x32[1], p_sad32x32[3], p_best_sad32x64[1],
        p_best_mv32x64[1]);
#endif
    SEARCH_CALC_BEST_16BIT_SAD(sad_32x16[0], sad_32x16[2], p_best_sad64x16[0],
        p_best_mv64x16[0]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_32x16[1], sad_32x16[3], p_best_sad64x16[1],
        p_best_mv64x16[1]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_32x16[4], sad_32x16[6], p_best_sad64x16[2],
        p_best_mv64x16[2]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_32x16[5], sad_32x16[7], p_best_sad64x16[3],
        p_best_mv64x16[3]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x32[0], sad_16x32[4], p_best_sad16x64[0],
        p_best_mv16x64[0]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x32[1], sad_16x32[5], p_best_sad16x64[1],
        p_best_mv16x64[1]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x32[2], sad_16x32[6], p_best_sad16x64[2],
        p_best_mv16x64[2]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x32[3], sad_16x32[7], p_best_sad16x64[3],
        p_best_mv16x64[3]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[0], sad_16x8[2], p_best_sad32x8[0],
        p_best_mv32x8[0]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[1], sad_16x8[3], p_best_sad32x8[1],
        p_best_mv32x8[1]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[4], sad_16x8[6], p_best_sad32x8[2],
        p_best_mv32x8[2]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[5], sad_16x8[7], p_best_sad32x8[3],
        p_best_mv32x8[3]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[8], sad_16x8[10], p_best_sad32x8[4],
        p_best_mv32x8[4]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[9], sad_16x8[11], p_best_sad32x8[5],
        p_best_mv32x8[5]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[12], sad_16x8[14], p_best_sad32x8[6],
        p_best_mv32x8[6]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[13], sad_16x8[15], p_best_sad32x8[7],
        p_best_mv32x8[7]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[16], sad_16x8[18], p_best_sad32x8[8],
        p_best_mv32x8[8]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[17], sad_16x8[19], p_best_sad32x8[9],
        p_best_mv32x8[9]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[20], sad_16x8[22], p_best_sad32x8[10],
        p_best_mv32x8[10]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[21], sad_16x8[23], p_best_sad32x8[11],
        p_best_mv32x8[11]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[24], sad_16x8[26], p_best_sad32x8[12],
        p_best_mv32x8[12]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[25], sad_16x8[27], p_best_sad32x8[13],
        p_best_mv32x8[13]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[28], sad_16x8[30], p_best_sad32x8[14],
        p_best_mv32x8[14]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_16x8[29], sad_16x8[31], p_best_sad32x8[15],
        p_best_mv32x8[15]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[0], sad_8x16[4], p_best_sad8x32[0],
        p_best_mv8x32[0]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[1], sad_8x16[5], p_best_sad8x32[1],
        p_best_mv8x32[1]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[2], sad_8x16[6], p_best_sad8x32[2],
        p_best_mv8x32[2]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[3], sad_8x16[7], p_best_sad8x32[3],
        p_best_mv8x32[3]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[8], sad_8x16[12], p_best_sad8x32[4],
        p_best_mv8x32[4]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[9], sad_8x16[13], p_best_sad8x32[5],
        p_best_mv8x32[5]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[10], sad_8x16[14], p_best_sad8x32[6],
        p_best_mv8x32[6]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[11], sad_8x16[15], p_best_sad8x32[7],
        p_best_mv8x32[7]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[16], sad_8x16[20], p_best_sad8x32[8],
        p_best_mv8x32[8]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[17], sad_8x16[21], p_best_sad8x32[9],
        p_best_mv8x32[9]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[18], sad_8x16[22], p_best_sad8x32[10],
        p_best_mv8x32[10]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[19], sad_8x16[23], p_best_sad8x32[11],
        p_best_mv8x32[11]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[24], sad_8x16[28], p_best_sad8x32[12],
        p_best_mv8x32[12]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[25], sad_8x16[29], p_best_sad8x32[13],
        p_best_mv8x32[13]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[26], sad_8x16[30], p_best_sad8x32[14],
        p_best_mv8x32[14]);
    SEARCH_CALC_BEST_16BIT_SAD(sad_8x16[27], sad_8x16[31], p_best_sad8x32[15],
        p_best_mv8x32[15]);

#undef SEARCH_CALC_BEST
#undef SEARCH_CALC_BEST_16BIT_SAD
#undef SEARCH_BEST
#undef SEARCH_BEST_16BIT_SAD
#undef ADD_VECS
#undef ADD_VECS_16bit_SAD
#undef ADD_VECS_16BIT_SAD_PACK
#undef LOAD_16BIT_PACK_2
}
...
static INLINE uint32_t sad_kernel_4xm_16bit_avx2(
    ...
    uint32_t  width)
{
    ...
    for (y = 0; y < height; y += 4) {
        ...
        src01 = _mm_castpd_si128(_mm_loadh_pd(_mm_castsi128_pd(src01),
            (double *)(const __m128i *) (src + src_stride)));
        ...
        ref01 = _mm_castpd_si128(_mm_loadh_pd(_mm_castsi128_pd(ref01),
            (double *)(const __m128i *) (ref + ref_stride)));
...
static INLINE uint32_t sad_kernel_8xm_16bit_avx2(
    uint16_t  *src,                            // input parameter, source samples Ptr
    uint32_t  src_stride,                      // input parameter, source stride
    uint16_t  *ref,                            // input parameter, reference samples Ptr
    uint32_t  ref_stride,                      // input parameter, reference stride
    ...
    uint32_t  width)
{
    ...
    for (y = 0; y < height; y+=2) {
        const __m256i src0 = _mm256_set_m128i(_mm_loadu_si128((const __m128i *)
            (src + src_stride)), _mm_loadu_si128((const __m128i *)src));
        const __m256i ref0 = _mm256_set_m128i(_mm_loadu_si128((const __m128i *)
            (ref + ref_stride)), _mm_loadu_si128((const __m128i *)ref));
...
static INLINE uint32_t sad_kernel_16xm_16bit_avx2(
    uint16_t  *src,                            // input parameter, source samples Ptr
    uint32_t  src_stride,                      // input parameter, source stride
    uint16_t  *ref,                            // input parameter, reference samples Ptr
    uint32_t  ref_stride,                      // input parameter, reference stride
    ...
    uint32_t  width)                          // input parameter, block width (N))
{
    ...
    if (width_offset) {
        sad += sad_kernel_16xm_16bit_avx2(src, src_stride,
            ref, ref_stride,height, width);
    }
    if ((width - width_offset) > 4) {
        sad += sad_kernel_8xm_16bit_avx2(src + width_offset, src_stride,
            ref + width_offset, ref_stride, height, 8);
        width_offset += 8;
    }
    if (width - width_offset) {
        assert((width - width_offset) == 4);
        sad += sad_kernel_4xm_16bit_avx2(src + width_offset, src_stride,
            ref + width_offset, ref_stride, height, 4);
    }
    return sad;
}
