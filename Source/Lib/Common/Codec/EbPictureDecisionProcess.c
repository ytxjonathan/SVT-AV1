...
#if M3_ENABLE_HME_FLAGS
    enc_mode_hme = ENC_M3;
#elif M1_ENABLE_HME_FLAGS
    enc_mode_hme = ENC_M1;
#endif
...
// These are the default settings to use
...
#if TEST_PIC_MULTI_PASS_PD_MODE_0
...
#elif TEST_PIC_MULTI_PASS_PD_MODE_1
        picture_control_set_ptr->pic_depth_mode = (picture_control_set_ptr->slice_type == I_SLICE) ? PIC_ALL_DEPTH_MODE : PIC_MULTI_PASS_PD_MODE_1;
#elif TEST_PIC_MULTI_PASS_PD_MODE_2
        picture_control_set_ptr->pic_depth_mode = (picture_control_set_ptr->slice_type == I_SLICE) ? PIC_ALL_DEPTH_MODE : PIC_MULTI_PASS_PD_MODE_2;
#elif TEST_PIC_MULTI_PASS_PD_MODE_3
...
#elif TEST_PIC_MULTI_PASS_PD_MODE_4
        picture_control_set_ptr->pic_depth_mode = (picture_control_set_ptr->slice_type == I_SLICE) ? PIC_ALL_DEPTH_MODE : PIC_MULTI_PASS_PD_MODE_3;
#endif
    ...
    */
    ...
    if (picture_control_set_ptr->slice_type != I_SLICE)
        if (sc_content_detected)
            picture_control_set_ptr->intra_pred_mode = (picture_control_set_ptr->temporal_layer_index == 0) ? 2 : 3;
        else
            picture_control_set_ptr->intra_pred_mode = (picture_control_set_ptr->temporal_layer_index == 0) ? 1 : 3;
#elif M2_INTRA_PRED_MODE
    if (picture_control_set_ptr->slice_type != I_SLICE)
        if (sc_content_detected)
            picture_control_set_ptr->intra_pred_mode = (picture_control_set_ptr->temporal_layer_index == 0) ? 1 : 2;
        else
            picture_control_set_ptr->intra_pred_mode = (picture_control_set_ptr->temporal_layer_index == 0) ? 0 : 3;
#endif
    ...
    if (picture_control_set_ptr->hierarchical_levels == 0) {
       ...
       // { 2, 4, 6, 0 }     // GOP Index 0 - Ref List 1
...
#if LOW_DELAY_TUNE
        av1_rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, flat_pred_struct[gop_i].ref_list0[2]);
        av1_rps->ref_poc_array[GOLD] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, flat_pred_struct[gop_i].ref_list0[3]);
#else
        av1_rps->ref_poc_array[LAST3] = av1_rps->ref_poc_array[LAST];
        av1_rps->ref_poc_array[GOLD] = av1_rps->ref_poc_array[LAST];
#endif
...
#if LOW_DELAY_TUNE
        av1_rps->ref_poc_array[ALT] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, flat_pred_struct[gop_i].ref_list1[2]);
#else
        av1_rps->ref_poc_array[ALT] = av1_rps->ref_poc_array[BWD];
#endif
        ...
        //{4 , 8, 0, 0 }      // GOP Index 0 - Ref List 1
            ...
            av1_rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
            av1_rps->ref_poc_array[GOLD] = av1_rps->ref_poc_array[LAST];
        ...
        //{ -1, 0, 0, 0 }     // GOP Index 1 - Ref List 1
            ...
            av1_rps->ref_poc_array[LAST2] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[1]);
            av1_rps->ref_poc_array[LAST3] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[2]);
            av1_rps->ref_poc_array[GOLD] = get_ref_poc(context_ptr, picture_control_set_ptr->picture_number, two_level_hierarchical_pred_struct[gop_i].ref_list0[3]);;
...
#if PRED_DEBUG
            av1_rps->ref_dpb_index[LAST2] = lay1_0_idx;
            av1_rps->ref_dpb_index[LAST3] = lay2_1_idx;
...
#if PRED_CHANGE_5L
#if PRED_DEBUG_L5
            if (picture_control_set_ptr->is_used_as_reference_flag && picture_index %2 != 0 )
#else
                            ...
                            for (uint8_t loop_index = 0; loop_index <= picture_control_set_ptr->is_alt_ref; loop_index++) {
                                    ...
                                    sequence_control_set_ptr->static_config.pred_structure == EB_PRED_RANDOM_ACCESS)
                                {
                                    if (picture_control_set_ptr->av1_ref_signal.ref_poc_array[ALT] >= picture_control_set_ptr->picture_number)
                                        ...
                                        picture_control_set_ptr->av1_cm->ref_frame_sign_bias[ALTREF_FRAME] = 0;

                                    if (picture_control_set_ptr->av1_ref_signal.ref_poc_array[ALT2] >= picture_control_set_ptr->picture_number)
                                        picture_control_set_ptr->av1_cm->ref_frame_sign_bias[ALTREF2_FRAME] = 1;
                                    else
                                        picture_control_set_ptr->av1_cm->ref_frame_sign_bias[ALTREF2_FRAME] = 0;

                                    if (picture_control_set_ptr->av1_ref_signal.ref_poc_array[BWD] >= picture_control_set_ptr->picture_number)
                                        picture_control_set_ptr->av1_cm->ref_frame_sign_bias[BWDREF_FRAME] = 1;
                                    else
