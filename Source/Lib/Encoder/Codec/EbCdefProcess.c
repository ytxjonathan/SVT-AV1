...
void cdef_seg_search(PictureControlSet *pcs_ptr, SequenceControlSet *scs_ptr,
    ...
    uint32_t                        x_seg_idx;
    uint32_t                        y_seg_idx;
    uint32_t picture_width_in_b64  = (pcs_ptr->parent_pcs_ptr->av1_cm->frm_size.frame_width + 64 - 1) / 64;
    uint32_t picture_height_in_b64 = (pcs_ptr->parent_pcs_ptr->av1_cm->frm_size.frame_height + 64 - 1) / 64;
