...
void interpolate_search_region_avc_chroma(
    ...
    uint8_t **pos_b_buffer_ch, uint8_t **pos_h_buffer_ch, uint8_t **pos_j_buffer_ch,
    ...
    uint32_t input_bit_depth) // input parameter, input sample bit depth
{
    ...
    if (search_area_width_for_asm) {
        // Cb
        avc_style_luma_interpolation_filter(search_region_buffer_cb -
                                            ...
                                            2);
        // Cr
        avc_style_luma_interpolation_filter(search_region_buffer_cr -
                                                (ME_FILTER_TAP >> 1) * interpolated_full_stride_ch -
                                                (ME_FILTER_TAP >> 1) + 1,
                                            interpolated_full_stride_ch,
                                            pos_b_buffer_ch[1],
    ...
    if (search_area_width_for_asm) {
        // Cb
        avc_style_luma_interpolation_filter(pos_b_buffer_ch[0] + interpolated_stride_ch,
                                            interpolated_stride_ch,
                                            pos_j_buffer_ch[0],
                                            interpolated_stride_ch,
                                            search_area_width_for_asm,
                                            ...
                                            8);
        // Cr
        avc_style_luma_interpolation_filter(pos_b_buffer_ch[1] + interpolated_stride_ch,
                                            interpolated_stride_ch,
                                            pos_j_buffer_ch[1],
                                            interpolated_stride_ch,
                                            search_area_width_for_asm,
...
// TODO: Alt-refs - change previous select_buffer and quarder_pel_compensation to
// be applicable for both chroma and luma
static void select_buffer_luma_chroma(uint32_t  pu_index, //[IN]
                                      EbBool    chroma,
                                      uint8_t   fracPosition, //[IN]
                                      uint32_t  pu_width, //[IN] Refrence picture list index
                                      uint32_t  pu_height, //[IN] Refrence picture index in the list
                                      ...
                                      uint32_t *DstPtrStride) //[OUT]
{
    ...
    if (chroma == EB_TRUE) {
        pu_shift_x_index = (pu_search_index_map[pu_index][0]) >> 1;
        pu_shift_y_index = (pu_search_index_map[pu_index][1]) >> 1;
    } else {
        pu_shift_x_index = pu_search_index_map[pu_index][0];
        pu_shift_y_index = pu_search_index_map[pu_index][1];
    }
 ...
 *******************************************************************************/
void uni_pred_averaging(uint32_t pu_index, EbBool chroma, uint8_t firstFracPos, uint32_t pu_width,
                        uint32_t pu_height, uint8_t *firstRefInteger, uint8_t *firstRefPosB,
                        uint8_t *firstRefPosh_, uint8_t *firstRefPosJ, uint32_t refBufferStride,
                        uint32_t refBufferFullList0Stride, uint8_t *firstRefTempDst,
                        uint8_t **comp_blk_ptr, uint32_t *comp_blk_ptr_stride) {
    // Buffer Selection and quater-pel compensation on the fly
    if (sub_position_type[firstFracPos] != 2) {
        select_buffer_luma_chroma(pu_index,
                                  chroma,
                                  firstFracPos,
                                  pu_width,
                                  pu_height,
...
/*******************************************
 * sixteenth_decimated_search
 *  performs a 1/16 decimated search
 *******************************************/
uint64_t sixteenth_decimated_search(MeContext *context_ptr, int16_t origin_x, int16_t origin_y,
    ...
    if ((search_area_width & 15) == 0) {
        ...
        // other width sizes work with the old code as the one
        // in"sad_loop_kernel_sse4_1_intrin"
        sad_loop_kernel_hme_l0(&context_ptr->sixteenth_sb_buffer[0],
