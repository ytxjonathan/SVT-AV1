    ...
    uint32_t cu_qp) //input parameter, Qp of the CU
{
    ...
    uint32_t qp_array_idx =
        (cuPos_y / MIN_BLOCK_SIZE) * pcs_ptr->qp_array_stride + (cuPos_x / MIN_BLOCK_SIZE);

    for (vertical_idx = 0; vertical_idx < cu_size_in_min_blk_size; ++vertical_idx) {
        EB_MEMSET(pcs_ptr->qp_array + qp_array_idx + vertical_idx * pcs_ptr->qp_array_stride,
                  cu_qp,
                  sizeof(uint8_t) * cu_size_in_min_blk_size);
    }
