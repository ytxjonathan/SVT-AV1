...
void av1_nn_output_prec_reduce(float *const output, int num_output) {
    ...
    for (int i = 0; i < num_output; i++) { output[i] = ((int)(output[i] * prec + 0.5)) * inv_prec; }
}

/*!< Calculate prediction based on the given input features and neural net config.
 *   Assume there are no more than NN_MAX_NODES_PER_LAYER nodes in each hidden layer. */
void av1_nn_predict_c(const float *input_nodes, const NnConfig *const nn_config, int reduce_prec,
                      float *const output) {
