...
EbErrorType av1_split_flag_rate(PictureParentControlSet *pcs_ptr, ModeDecisionContext *context_ptr,
    ...
    if (is_partition_point) {
        ...
        const int32_t has_rows = (blk_origin_y + hbs) < pcs_ptr->av1_cm->frm_size.frame_width;
        const int32_t has_cols = (blk_origin_x + hbs) < pcs_ptr->av1_cm->frm_size.frame_height;
